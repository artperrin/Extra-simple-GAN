...
def plot_data(data: list) -> None:
    data = np.array(data)
    _, ax = plt.subplots()
    ax.scatter(data[:, 0], data[:, 1], color='blue', alpha=.5)
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    return ax
...
def main():
    ...
    sampler = sample_data(100, gen, scale=1)
    # ax = plot_data(list(sampler))
    sampler = sample_data(100, gen, scale=1)
    dataset = dt.DataLoader(database.Samples(sampler), batch_size=128)
    neuralnetwork.train_gan(dataset, neuralnetwork.Generator(10, 2), neuralnetwork.Discriminator(2, 2), epochs=100001, lr=cst.LEARNING_RATE) # write_graph=True, n_test_graph=100)
